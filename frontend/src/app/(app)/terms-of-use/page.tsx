...
import { termOfUseType } from "@/lib/types";
import React from "react";

const TermOfUsePage = async () => {
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_API_URL}/api/legal/terms-of-use`,
    {
  ...
  const data: termOfUseType = await res.json();

  return (
    <MaxWidthWrapper className="py-12">
      <div className="max-w-4xl px-2 py-6">
        <h1 className="text-4xl font-black mb-10">
          Term of <span className="text-yellow-600">Use</span>
        </h1>
